...
def get_contact(db: Session, contact_id: int):
    return db.query(database.ContactDB).filter(database.ContactDB.id == contact_id).first()


def get_contacts(db: Session, skip: int = 0, limit: int = 100, first_name: str = None,
...
def update_contact(db: Session, contact_id: int, contact: models.ContactUpdate):
    db_contact = db.query(database.ContactDB).filter(database.ContactDB.id == contact_id).first()
    if db_contact:
...
def get_upcoming_birthdays(db: Session):
    ...
    for contact in contacts:
        # Порівнюємо день та місяць народження з поточним та наступним тижнем
        birthday_this_year = date(today.year, contact.birthday.month, contact.birthday.day)
        if today <= birthday_this_year <= next_week:
            upcoming_birthdays.append(contact)
        else:
            birthday_next_year = date(today.year + 1, contact.birthday.month, contact.birthday.day)
